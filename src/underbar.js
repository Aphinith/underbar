(function() {
  ...
  _.each = function(collection, iterator) {
    //conditional statement that determines whether collection is an array or not
    if (Array.isArray(collection)) {
      //create for loop to iterate through collection
      for (var i = 0; i < collection.length; i++) {
        //call iterator on collection values and index and collection itself
        iterator(collection[i], i, collection);
      }
    } else {
      //loop through object
      for (var key in collection) {
        //call iterator on collection
  ...
  _.filter = function(collection, test) {
    //create an varible to hold results
    var filtered = [];
    //loop thru collection
    _.each(collection, function(val){
      //test to see if each value passes the test
      if(test(val)){
        filtered.push(val);
      }
    })
  ...
  _.reject = function(collection, test) {
    ...
    var rejected = _.filter(collection, function(val){
      //returns failed test
      return !test(val)
    })
    return rejected;
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(val){
      //call the iterator on val push it to result array
  ...
  _.reduce = function(collection, iterator, accumulator) {
    //if accumulator is undefined assign start as collection[1]
    if (accumulator === 0) {
      var start = 0;
    } else if (accumulator === undefined) {
      //if accumulator is undefined, then set start equal to accumulator
    ...
    //run iterator through collection
    _.each(collection, function(val) {
      //assign the result of iterator to start
      start = iterator(start, val);
    })
    return start;
